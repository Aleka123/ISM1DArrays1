...
namespace Arrays2Func
{
    class Program
    {
        ...
        static double BetweenAbs(double[] arr)
        {
            double rez = 1;
            int a = 0, b = 0;
        ...
        static double BetweenMinAndMax(double[] arr)
        {
            ...
            if (k > l) for ( i = l + 1; i < k; i++) sum += arr[i];
            else for ( i = k + 1; i < l; i++) sum += arr[i];
            return sum;
        }
        static double AfterMinElement(double[] arr)
        {
        ...
        static void Main(string[] args)
        {
            ...
            WriteArray(arr);
            double rez1 = AfterMinElement(arr);
            if (rez1 == 1) Console.WriteLine("Нет элементов после мин. ");
            else Console.WriteLine("Произведение элементов, после мин: {0}", rez1);
            double sum1 = BetweenMinAndMax(arr);
            Console.WriteLine("Сума элементов, между первым отрицательным и вторым положительным элементами: {0}", sum1);
            double rez2 = BetweenAbs(arr);
            if (rez2 > 1 || rez2 < 1)
                Console.WriteLine("Произведение элеметнтов, находящихся между мин и макс по модулям элементами: {0}", rez2);
            else Console.WriteLine("Нет элементов между мин и макс по модулям элементами.");
            double sum2 = BetweenZeros(arr);
            Console.WriteLine("Сумма элементов, между первым и последним нулевыми элементами: {0}", sum2);
        }
    }
}